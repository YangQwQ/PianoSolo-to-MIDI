...
from torch_geometric.utils import scatter as scatter_add_compat
...
def compute_voice_f1_score(pred_edges, truth_edges, num_nodes):
    ...
    multiplicity_pred = scatter_add_compat(ones_pred, pred_edges[0], dim=0, dim_size=num_nodes, reduce='sum') + 1e-8
    weights_pred = multiplicity_pred[pred_edges[0]]
    multiplicity_true = scatter_add_compat(ones_true, truth_edges[0], dim=0, dim_size=num_nodes, reduce='sum') + 1e-8
